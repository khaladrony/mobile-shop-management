...
@RestController
@RequestMapping("/application_common/supplier_info")
public class SupplierInfoController extends AppProperty {

    @Autowired
    AppUtil appUtil;
    @Autowired
    SupplierInfoActionService supplierInfoActionService;
    @Autowired
    ...
    public AppResponse<BankInfo> save(@RequestBody SupplierInfo supplierInfo){
        return supplierInfoActionService.execute(supplierInfo);
    }

    @RequestMapping(value = "/update", method = RequestMethod.PUT)
    public AppResponse<BankInfo> update(@RequestBody SupplierInfo supplierInfo) {
        return supplierInfoActionService.execute(supplierInfo);
    }

    @RequestMapping(value = "/get/{id}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    public AppResponse<Object> get(@PathVariable("id") long id) {
        SupplierInfo model = supplierInfoService.findById(id);
        if (model != null) {
    ...
    public AppResponse<Object> filter(
        ...
        try {
            return AppResponse.build(HttpStatus.OK).body(supplierInfoService.filter(currentPage, itemPerPage, params));
        } catch (Exception ex) {
    ...
    public AppResponse<Object> getSupplierList() {
        try {
            ...
            if (!list.isEmpty()) {
                ...
                return AppResponse.build(HttpStatus.NO_CONTENT).message("Supplier not found");
            }
        } catch (Exception ex) {
            return AppResponse.build(HttpStatus.INTERNAL_SERVER_ERROR).message(ex.getMessage());
        }
