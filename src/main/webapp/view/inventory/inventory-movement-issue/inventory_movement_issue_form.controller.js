app.controller('InventoryMovementIssueFormCtrl', function ($scope, $http, $state, $timeout,
    ...
    $scope.saveModule = function () {
        ...
        req.then(function (resp) {
            log("Inventory movement issue: " + JSON.stringify(resp));
            if (resp.code === 200) {
                $scope.reset();
                $scope.module.details = [];

                if($state.current.name === JCOMPONENT.inventory_movement_issue_update_view){
                    $state.go(JCOMPONENT.inventory_movement_issue_list_view);
            ...
            log("Inventory movement issue save error", JSON.stringify(err));
    ...
    if($state.current.name === JCOMPONENT.inventory_movement_issue_update_view){
        ...
        req.then(function (resp) {
            log("Issue transaction edit: " + JSON.stringify(resp));
            ...
            log("Issue transaction edit error", JSON.stringify(err));
        });
    }
