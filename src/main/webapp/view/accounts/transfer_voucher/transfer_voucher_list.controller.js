app.controller('AccTransferVoucherListCtrl', function ($scope, $http, $state, $timeout, $rootScope, $mdDialog, $interval, DialogBox, Communication,growl) {
    ...
    $scope.getDataList = function (currentPage, itemPerPage) {
        ...
        var req = Communication.request("POST", API.ACC_TRANSFER_VOUCHER_FILTER + "/" + currentPage + "/" + itemPerPage, $scope.search);
        req.then(function (resp) {
            DialogBox.hideProgress();
            log("transfer voucher list: " + JSON.stringify(resp));
    ...
    $scope.getVoucherNoList = function () {
        ...
        $scope.voucherNoList = [];
        var req = Communication.request("GET", API.ACC_TRANSFER_VOUCHER_LIST + "/" + $scope.search.voucher_type, {});
        req.then(function (resp) {
            log("transfer voucher no list: " + JSON.stringify(resp));
            if (resp.code === 200) {
                $scope.voucherNoList = resp.body;
                $scope.getVoucherStatusList();
            }
        }, function (err) {
            log("transfer voucher no list error", JSON.stringify(err));
        });
    };
