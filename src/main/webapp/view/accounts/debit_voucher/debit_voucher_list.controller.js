app.controller('AccDebitVoucherListCtrl', function ($scope, $http, $state, $timeout, $rootScope, $mdDialog, DialogBox, $interval, Communication,growl) {
    ...
    $scope.search = {
        ...
        payment_type: "",
        status: "",
        voucher_type: VOUCHER_KEY.TYPE.DEBIT_VOUCHER
    };
    ...
    $scope.getDataList = function (currentPage, itemPerPage) {
        ...
        var req = Communication.request("POST", API.ACC_DEBIT_VOUCHER_FILTER + "/" + currentPage + "/" + itemPerPage, $scope.search);
    ...
    $scope.getVoucherNoList = function () {
        ...
        $scope.voucherNoList = [];
        var req = Communication.request("GET", API.ACC_DEBIT_VOUCHER_LIST + "/" + $scope.search.voucher_type, {});
        req.then(function (resp) {
            log("Debit voucher no list: " + JSON.stringify(resp));
            if (resp.code === 200) {
                $scope.voucherNoList = resp.body;
                $scope.getVoucherStatusList();
            }
        }, function (err) {
            log("Debit voucher no list error", JSON.stringify(err));
        });
    };
    ...
    $scope.showEditForm = function (obj) {
        $state.go(JCOMPONENT.acc_debit_voucher_update_view, {id: obj.id});
    };
    ...
    $scope.voucherPreview = function (id) {
        ...
        var fileName = "voucherViewA5"; //jrxml file name
