app.controller('AccDebitVoucherFormCtrl', function ($scope, $http, $state, $timeout, $stateParams, $rootScope, $sce, $mdDialog, $interval, ClientService, DialogBox, encrypt, Communication,$filter,growl) {
    ...
    $scope.module = {
        ...
        voucherPrefix: VOUCHER_KEY.PREFIX.DEBIT_VOUCHER,
        chequeNo: "",
        chequeDate: "",
        chequeStatus: "",
        action: "",
    ...
    $scope.saveModule = function () {
        ...
        var req;
        if($state.current.name === JCOMPONENT.acc_debit_voucher_update_view){
            req = Communication.request("PUT", API.ACC_DEBIT_VOUCHER_UPDATE, $scope.module);
        } else{
            req = Communication.request("POST", API.ACC_DEBIT_VOUCHER_SAVE, $scope.module);
        }

        req.then(function (resp) {
            log("Debit voucher: " + JSON.stringify(resp));
            if (resp.code === 200) {
                ...
                if($state.current.name === JCOMPONENT.acc_debit_voucher_update_view){
                    $state.go(JCOMPONENT.acc_debit_voucher_list_view);
    ...
    if($state.current.name === JCOMPONENT.acc_debit_voucher_update_view){
        ...
        var req = Communication.request("GET", API.ACC_DEBIT_VOUCHER_GET + '/' + $stateParams.id, $scope.module);
        req.then(function (resp) {
