app.controller('AccJournalVoucherListCtrl', function ($scope, $http, $state, $timeout, $rootScope, $mdDialog, DialogBox, $interval, Communication,growl) {
    ...
    $scope.search = {
        ...
        payment_type: "",
        status: "",
        voucher_type: VOUCHER_KEY.TYPE.JOURNAL_VOUCHER
    };
    ...
    $scope.getDataList = function (currentPage, itemPerPage) {
        ...
        var req = Communication.request("POST", API.ACC_JOURNAL_VOUCHER_FILTER + "/" + currentPage + "/" + itemPerPage, $scope.search);
        req.then(function (resp) {
            DialogBox.hideProgress();
            log("Journal voucher list: " + JSON.stringify(resp));
    ...
    $scope.getVoucherNoList = function () {
        ...
        $scope.voucherNoList = [];
        var req = Communication.request("GET", API.ACC_JOURNAL_VOUCHER_LIST + "/" + $scope.search.voucher_type, {});
        req.then(function (resp) {
            log("Journal voucher no list: " + JSON.stringify(resp));
            if (resp.code === 200) {
                $scope.voucherNoList = resp.body;
                $scope.getVoucherStatusList();
            }
        }, function (err) {
            log("Journal voucher no list error", JSON.stringify(err));
        });
    };
    ...
    $scope.showEditForm = function (obj) {
        $state.go(JCOMPONENT.acc_journal_voucher_update_view, {id: obj.id});
    };

    $scope.voucherCreate = function () {
        $state.go(JCOMPONENT.acc_journal_voucher_add_view);
    };
